...
class banner extends common {
	...
	public function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(D('banner')->insert($_POST, true)){
                return_json(array('status'=>1,'icon'=>1,'message'=>L('operation_success')));
			}else{
                return_json(array('status'=>0,'icon'=>2,'message'=>L('operation_failure')));
			}
		}else{
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			if(D('banner')->update($_POST, array('id'=>$_POST['id']))){
                return_json(array('status'=>1,'icon'=>1,'message'=>L('operation_success')));
			}else{
                return_json(array('status'=>0,'icon'=>2,'message'=>L('operation_failure')));
			}
