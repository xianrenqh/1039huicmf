...
switch ($step) {
    ...
    case '4':
        if (isset($_GET['install'])) {
            ...
            $dbPrefix = empty($_POST['dbprefix']) ? 'hui_' : str_replace(array('"','\'',','), '', trim($_POST['dbprefix']));
            ...
            $time = time();
            $query = "INSERT INTO `{$dbPrefix}admin` VALUES ('1', '{$adminname}', '{$password}', '1', '超级管理员', '', '', null , '', {$time},'1', '1')";
...
function sql_split($sql, $tablepre) {
    ...
    if ($tablepre != "hui_")
        $sql = str_replace("hui_", $tablepre, $sql);
